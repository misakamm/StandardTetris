...
namespace CPF
{
	namespace StandardTetris
	{
		...
		string STStrategyUserDefined::GetStrategyName ( )
        {
            return ((string)"User Defined");
        }
		...
		void STStrategyUserDefined::GetBestMoveOncePerPiece
        (
            STBoard & board,
            STPiece & piece,
        ...
        )
		{
			...
			STStrategyUserDefined::PrivateStrategy
				(
		...
		double STStrategyUserDefined::PrivateStrategy
			( 
			...
			)
		{
			...
			if (0 == board.IsValid()) return(0.0);
			if (0 == piece.IsValid()) return(0.0);
			...
			double currentBestMerit = (-1.0e+20f);
				...
				if (0 != moveIsPossible)
				{
						...
						)
					{
						...
						if (0 != moveAcceptable)
						{
							...
							STStrategyUserDefined::PrivateStrategyEvaluate
							( 
								tempBoard,
								tempPiece,
								trialMerit
		...
		void  STStrategyUserDefined::PrivateStrategyEvaluate
			( 
			STBoard &  board,
			STPiece &  piece,
			double & rating
